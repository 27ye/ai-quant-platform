...
```http
POST /api/v1/stocks/catalog/sync
```

显式触发一次目录同步（拉取全市场目录 → 校验 → 落库并更新同步状态）：
...
```json
{
  ...
  "data": {
    ...
    "synced_at": "2026-09-22T10:33:12.345678+00:00",
    "catalog_complete": true
  }
}
```

- **这是 V3 补上的触发入口**：此前 `StockCatalogService.sync()` 只有 CLI（`scripts/sync_stock_catalog.py`）能调用，
  **运行中的服务没有任何端点或启动钩子触发它**，因此真实后端启动后目录恒为空、搜索恒 `50006`。
- **同步仍不在搜索路径里**：`GET /stocks/search` 只读本地目录、永不调用 Provider；本端点只是把「谁来触发同步」
  变成一次显式动作，不改变 4.1 的契约。
- **可重复调用＝刷新**，不是追加：重复调用只更新目录内容与 `last_success_at`。
- **失败如实上报**：Provider 失败 → `50001`（HTTP 502）并记录 `last_error`；**已成功同步过的目录仍然可搜索**，
  只有**从未成功同步**才返回 `50006`。校验不通过（例如只取到几十行）同样按失败处理，不把残缺目录当成「小市场」。
- 部署/验收建议：环境初始化后调用一次本端点（或继续用 CLI）再做搜索验收；**不建议**把 Provider 调用放进首屏请求。
