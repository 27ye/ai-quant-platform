...
if args.mysql:
    ...
    db='ai_quant_cruntimef465_'+datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S_%f')
...
engine.dispose()
out={'sha':'f465ee3b0e96c7787c6d5102da4f51455b76905d','database_version':version,'mode':'synthetic Provider through real StockService and MarketDataService; no live data','existing_databases_modified':False,'diagnostics_completed':True,'results':results,'all_contracts_met':all(x['contract_met'] for x in results.values())}
