...
report = {'d_sha': 'bf7cd54a7694728e48937ce9d8f54a5ae4a12714',
...
if '--with-c-overlay' in sys.argv:
    report['c_code_sha'] = '6a84901c82256b7d48dfab3ebee3208633e5058c'
    report['scope'] = report['scope'].replace('D22 unmodified checkout', 'D22 plus C18 quant-only overlay (no B21)')
with tempfile.TemporaryDirectory(prefix='c-d22-review-') as folder:
    ...
    with ExitStack() as stack:
        ...
        for field, replacement in (('total_return', .99), ('max_drawdown', -.5), ('trade_count', 999)):
            backtest_id = save(direct)
            with Session(engine) as db:
            ...
            report['corruption_probes'].append({'field': field, 'original': original, 'replacement': replacement,
                'http_status': response.status_code, 'code': response.json()['code'],
