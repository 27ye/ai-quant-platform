...
if args.mysql:
    ...
    db='ai_quant_cruntimebcbd_'+datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S_%f')
...
# A start-only guard cannot detect missing middle or tail dates when coverage is unknown.
for code,returned in [('600016',[6,8,9]),('600017',[6,7])]:
    with Session(engine) as s:
        repo=MarketDataRepository(s);seed(repo,[bar(code,f'2025-01-0{n}') for n in [6,7,8,9]])
        calendar=TradingCalendarProvider(trade_dates=[date(2025,1,n) for n in [7,8,9,10]])
        p=Provider([bar(code,f'2025-01-0{n}',101) for n in returned]);rejected=False
        try:MarketDataService(StockService(p),repo,calendar.count_between,lambda:date(2025,1,9)).query_daily(code,date(2025,1,6),date(2025,1,9),min_rows=2)
        except StockDataProviderError:rejected=True
    with Session(engine) as s:
        repo=MarketDataRepository(s);rows=repo.list_daily(code);state=repo.get_daily_sync(code)
        preserved=[r.trade_date.day for r in rows]==[6,7,8,9] and all(r.close==100 for r in rows) and state.source==OLD
        results['unknown_calendar_'+('middle' if code=='600016' else 'tail')]={'contract_met':rejected and preserved,'rejected':rejected,'old_snapshot_preserved':preserved,'provider_calls':p.calls,'remaining_dates':[str(r.trade_date) for r in rows],'same_first_date':bool(rows) and rows[0].trade_date==date(2025,1,6)}
engine.dispose()
tested_sha=subprocess.check_output(['git','-C',str(args.source.resolve()),'rev-parse','HEAD']).decode().strip()
out={'sha':tested_sha,'database_version':version,'mode':'synthetic Provider through real StockService and MarketDataService; no live data','existing_databases_modified':False,'diagnostics_completed':True,'results':results,'all_contracts_met':all(x['contract_met'] for x in results.values())}
