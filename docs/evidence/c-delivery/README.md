...
`docs/examples/C_V2_B_DELIVERY_HASHES_20260916.json`（C 发布）保存的是**原公布的 CRLF
字节哈希**（如 600519 normalized 的 `75caedb8…`），所以它**现在可直接校验本目录的文件**，
不需要做任何 LF↔CRLF 转换。若要核对 `ce06ed3` 时代的旧 LF blob，转换关系是
「把文件所有 CRLF 换成 LF」。
...
normalized_source = MarketDataService(...)      # 走数据库侧，可能直接返回旧缓存
normalized_rows = normalized_source.query_daily(code, start, end, ...)
```

本批导出发生在 **UTC `2026-09-16 00:05:41`（北京 9/16 08:05）**，此时 `2026-09-15`
**已经是已收盘的历史交易日**。三只股票的库内 provenance（`mysql_readback.json`，原样未改）
解释了差异：

| 股票 | 库内 `last_success_at`（UTC） | 折算北京时间 | 库内 9/15 那一行的来源 | 结果 |
|---|---|---|---|---|
| 600519 | `2026-09-15 01:46:21` | **9/15 09:46**（开盘约 16 分钟后） | 盘中**未完成**快照，此后未再刷新 | **不一致** |
| 000001 | `2026-09-15 07:26:22` | 9/15 15:26（收盘后） | 9/15 已结算收盘值 | 只差舍入 |
| 300750 | `2026-09-16 00:05:46` | 9/16 08:05（导出当刻刚同步） | 导出时刚取到 | 只差舍入 |

即 `normalized` 走库内缓存，而 **600519 的库内 9/15 行停留在当天 09:46 的盘中状态**；
导出当刻 `raw` 直连 provider 拿到的是 9/15 的**结算值**。两批快照不同，六项字段因此不同。

成交量方向上 raw(`13762`) > normalized(`2942`) 与该结论一致，但**判断依据是上表的
metadata 时间戳，不是成交量大小**；具体是哪一次请求命中旧缓存仍需日志才能逐一坐实。

> **更正**：本目录 README 的上一版曾写「`2026-09-15` 是抓取当天、当时尚未收盘」——
> 与上表元信息不符（抓取发生在 **9/16**，9/15 当时已收盘）。此处以原始元信息为准，
> 原 `*_metadata.json` / `mysql_readback.json` **未做任何修改**。

**这是导出脚本的取数口径缺陷（双源 + 旧缓存），不是计算问题。** 该日不在九组验收区间内
（`2025-07-04 ~ 2026-08-31`），窗口内数据两条路径完全一致。
