...
1. ~~待评审 / 合并~~ → 已按 D 的阶段计划并入 **PR #10**（唯一集成入口）。契约问题按 `docs/V2_B_DATA_CONTRACT.md` §8 定稿；目录完整性口径维持 **≥1000 行**（C 已确认不再作为修复阻塞）。
2. ~~等 C1 定稿~~ → C1 已定稿；白名单 `2 ≤ period ≤ 120`、预热行数 `max(ma_trend_period, ma_long_period+1)` 与 C 侧一致（C 已复验）。
