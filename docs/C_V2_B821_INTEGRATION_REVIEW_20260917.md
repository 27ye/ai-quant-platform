...
## 后续更新：B a7cfa91 预热说明关闭

已核对 B `a7cfa916ddb57483c02dac9138561afd53192e2e` 及 [执行回复](https://github.com/27ye/ai-quant-platform/issues/11#issuecomment-5707598367)。该提交父项为821f322，仅修改 `docs/backend_status.md`（+6/-2），代码与数据包不变。V2 开始日前最后long条、数据包覆盖≥120条的区别，以及此前六项待集成清单均已写清；关闭本轮提出的 B 预热/集成状态文档待办。此前测试保留原SHA，不因纯文档变更重跑或改签。

回复里“B树内没有跨字段检查”的表述需澄清：B 的 `backtest_service.resolve_effective_parameters` 第125–127行已有 `short >= long` 拒绝检查；C仍是完整参数契约的校验入口。文档所谓“分开传递区间”应理解为传完整预热+区间frame，并显式传start_date/end_date，由C选择窗口；不是拆成两份行情列表。本项只澄清说明，不要求再开发校验或裁剪逻辑，不新增最终验收门禁。

D 当前仍为b480014。建议完整合入 **B a7cfa91**，包括此前六提交及本次文档更正；其余C工具、A哈希标签、最终SHA与签字待办不变。[关闭证据](evidence/c-b821-20260917/B-a7cfa91-closure.json)

下文保留 B821 时点核对结果，B预热待办以本节后续状态为准。
