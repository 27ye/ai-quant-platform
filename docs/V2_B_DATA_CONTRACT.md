...
### 2.1 运行时兜底（腾讯 qfq）—— D 集成现状，见 Issue #15

D 在 **Issue #15** 提案「东财失败后腾讯 qfq 兜底」；C 已回该 Issue 的 `5723566029` 复现性契约；B 已对齐。
B 侧核实的前两项风险已由 D 在 `0c66c7d` 修正；下表保留原始诊断与处理口径：

| 风险 | 位置（已核实） | 要求 |
|---|---|---|
| 派生 `change_pct` 被**二次除 100** | Normalizer 对来源百分数字段统一 `/100`，腾讯派生值本来已是小数 | 腾讯适配时先转成百分数，再交给现有 normalizer；`100→101` 的结果仍为 `0.01`，并已覆盖裁剪前计算与首日缺前值场景 |
| 来源标识失真 | `StockService.provider_name` 固定为包装器名 `"akshare"` | 运行时记录实际 K 线端点；live 读 `last_kline_source`，cache 读 `stock_daily_sync.source`，不再用包装器名冒充来源 |
