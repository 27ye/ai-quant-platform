...
### 2.1 运行时兜底（腾讯 qfq）—— 口径待决策，见 Issue #15

D 在 **Issue #15** 提案「东财失败后腾讯 qfq 兜底」；C 已回该 Issue 的 `5723566029` 复现性契约；B 已对齐。
B 侧**已核实两点、并由 B 负责修正**（两者都在 B 的文件里）：

| 风险 | 位置（已核实） | 要求 |
|---|---|---|
| 派生 `change_pct` 被**二次除 100** | `providers/akshare_provider.py:673-675` 对 `change_pct` 无条件 `/100`；而导出器派生的是**小数**（100→101 ⇒ `0.01`） | 只对「百分数来源」列除 100，派生列不再除。补回归：`100→101 ⇒ 0.01`、缺前值为 `null`、裁剪前计算 |
| 来源标识失真 | `StockService.provider_name` 返回**包装器类名**（固定 `"akshare"`），而 `get_query_provenance()` 读它 → 腾讯兜底时 AI 快照仍写 `akshare` | 改从 `last_kline_source`（真实端点）派生，并验证 live→cache→AI 快照→历史 GET 全链路保留真实厂商 |

**两项待 D/C 决策的口径**（B 不擅自改）：

1. **混源窗口**：`stock_daily` **无 per-row source 列**，`list_daily` 可能把切源前另一种厂商的行与新区间混进同一帧。
   方案 A ＝切源时重取覆盖该股整段（本次落地口径，B 倾向）；方案 B ＝加 per-row `source` 列（迁移 **v9**，长期）。
2. **完整性基准**：`len(cached) < expected` 把**今天**计入 `expected`，盘中缓存永远不可能"完整"，必然回源
   （上游不可用时即整天 `50001`）。是否改为「基准 = 最后一个**已结算**交易日」需 D/C 同意——**属放宽判据，不擅自动**。

不改评分/校验口径与 4/2/6；腾讯仅在失败时兜底；来源如实标记、不得用 `0` 代替缺失；全部失败仍 `50001`。
