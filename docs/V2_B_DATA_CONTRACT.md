...
| `push2delay` kline | ❌ 200 但 `dktotal=0` —— **2026-09-17 实测 5 天 / 1 月 / 1 年 / 全量四种窗口全部返回空** | **该兜底当前在任何长度下都无效**（早期只在长区间观察到，现已确认是端点级而非长度相关） | 空响应抛 `50001`，不伪装成空数据；D 已在 **Issue #15** 提案用腾讯 qfq **替换这条 kline 兜底**，B 已答复同意方向（详见 Issue #15），最终由 D/C 决策 |
...
### 2.1 运行时兜底（腾讯 qfq）—— 已实现并复验（`fe8c0b4` / `bcbd559`）

D 在 **Issue #15** 提案「东财失败后腾讯 qfq 兜底」；C 的复现性契约见 `5723566029`，
**C 的两项决策见 `5724159704`（已确认）**；B 于 `5723849963` 对齐。

**B 提出的两项风险：D 已修，B 已核验**

| 风险 | 位置 | 处置（已核验） |
|---|---|---|
| 派生 `change_pct` 被**二次除 100** | `providers/akshare_provider.py` 对来源百分数列统一 `/100` | D 在 `0c66c7d` 改为**先转百分数再交给既有 normalizer**；C 在 `be35e5f` 复验 `100→101 ⇒ 0.01`、首行 `null`、裁剪前派生**通过** |
| 来源标识失真 | `StockService.provider_name` 仍返回包装器名（未改，也不需要改） | D 改的是**消费方**：`get_query_provenance()` 现在 `live` 读 `last_kline_source`（真实端点）、`cache` 读 `stock_daily_sync.source`、否则 `unknown`，仅最后兜底用 `provider_name`。B 已读 `be35e5f` 源码核验 |

**C 已确认的两项口径**（`5724159704`）

1. **方案 A：切源时重取覆盖**——采用；**暂不要求 v9**（不加 per-row `source` 列）。
   验收边界：覆盖「该股既有有效缓存 ∪ 本次预热/计算窗口」的**并集**，保证同源与 qfq 连续；
   要处理**双向切源**与**来源不明的旧缓存**；同厂商复权基准变化也不能留下不连续前缀。
2. **完整性基准 = 最近已完成交易日**——**属语义修正，不是放宽**；同一"已完成区间"的完整性要求不放宽。
   用 Asia/Shanghai + 可信交易日历 + 明确的收盘后可用规则确定期望截至日，**不能拿库里最后一天反推**；
   周末/假期回退到上一已完成交易日；**日历未知仍保守**。

**实现时必须满足（C 的约定 + C 用合成探针复现的三处缺口）**

| # | 缺口（C 已复现） | 要求 |
|---|---|---|
| 1 | 旧来源前 2 行 + 腾讯后 2 行组成"完整缓存"，`query_daily` 整段返回并把 provider 标为腾讯——**只读 `stock_daily_sync.source` 消除不了混源** | 按方案 A 整段替换；来源**从返回批次**传到 cache/AI/历史，**不能取进程的"最近一次来源"** |
| 2 | 在"写成功来源"阶段注入失败：重建会话后**新价格 101 可见、来源仍是旧的**，旧成功价 100 未恢复 | **行情与成功来源/范围必须原子发布**：先取全 → 校验 → 再原子发布；失败保留上次成功的数据与来源（失败可另记）；不动已保存报告/输入快照/冻结包 |
| 3 | 模拟 09-18 盘中：缓存到 09-17 仍因 `expected` 含 09-18 而回源；**含 09-18 未完成 bar 的响应会被接受** | 落地"完整日线边界"；**不能用盘中行凑完整日线数量**；期望边界推进后上游无数据须如实报未更新/失败，**不偷偷退回旧缓存日期**；请求截止日与实际数据截至日分别明确、随快照保存 |

**其它**（不变）：上游条数限制/缺日导致无法整段覆盖时，**不能把"尾部成功"写成"全段成功"**；
缓存命中 ≠ 实时门禁通过；全部上游失败且缓存不可用仍按既定 `50001`；不改评分/校验口径与 4/2/6；
腾讯仅在失败时兜底、来源如实标记、**不得用 `0` 代替缺失**。

**落地结果（2026-09-22）**

| 项 | 结论 |
|---|---|
| 实现 | D 在 **`fe8c0b4`** 落地：① 完成日上限贯穿 union 扩宽（`fetch_end = min(fetch_end, completed_through)`，放在 union 之后）、取数前先算 `expected`、越界行在归一化前过滤；② **`expected is None`（无法证明 union 覆盖）即抛 `50001`，且发生在 Provider 取数之前**，保留原成功行情与来源 |
| 复验 | C 在 **`bcbd559` + `c-delivery-20260917`** 签字：固定九组 9/9、SQLite/MySQL 运行时探针各 **12/12**、迁移与原子回滚通过；B 独立复审 `fe8c0b4`/`c30f799` 通过（B 自写回归对 `119a10c` 失败、对 `bcbd559` 通过；B 树 559 = 原 557 + B 的 2 条额外回归，C 记录仍为原树 557） |

**两处必须分别说明的失败原因**（C `5769755860` 第 4 点，不得混为一谈、不得包装成已通过）：

| # | 原因 | 语义 |
|---|---|---|
| a | **日历无法证明 union 覆盖**（`count_between` 返回 `None`） | **设计性拒绝**：取数前 `50001`，保留旧快照。是有意行为，不是缺陷 |
| b | **已知应有交易日、但上游缺该日** | **数据不完整**：如实报未更新/失败；**不得**把上游不完整说成已通过 |

> **有意取舍（请勿当新 bug 去"修"）**：规则 ② 落地后，"无法证明覆盖"从**保守重拉**变成**直接失败**，
> 这类情况下的可用性下降是 C 已确认的代价——**宁可 `50001` 也不换数据**。
...
| 旧缓存 | 新批次 | `cb2cfeb`（起点判据） | `bcbd559`（覆盖证明） |
|---|---|---|---|
| 01-06/07/08/09 | 06/08/09 | 替换成功，**07 中间行丢失** | 拒绝，旧四行与来源保留 |
| 01-06/07/08/09 | 06/07 | 替换成功，**08/09 末端丢失** | 拒绝，旧四行与来源保留 |

即"同起点"不能证明**中间**完整，"末端"在 `expected is None` 时也没有兜底。
**该分支已作废并从 fork 删除**，不再并行两套实现。
...
## 8. 原待评审问题 —— 均已定稿（2026-09-17 收口）

五个问题都已解决，此处保留原问与**结论依据**，便于追溯（不再是待办）：

| # | 原问题 | 结论 | 依据 |
|---|---|---|---|
| 1 | 目录同步完整性：≥1000 条 还是与上游 `total` 一致？ | **维持 ≥1000 条**（`MIN_CATALOG_ROWS`） | 上游 `total` 会漂移（停牌/退市），用它做判据会把一次正常刷新误判为失败；C 已确认"不新增与动态 total 严格相等的条件" |
| 2 | 同步触发：仅手动，还是启动时懒同步？ | **仅手动脚本 + 显式状态暴露** | 实现为 `scripts/sync_stock_catalog.py` + `GET /stocks/{code}/data-status` |
| 3 | `40005` / `40006` 分工 | `40005 = backtest not found`（B）、`40006 = report not found`（D），不复用同一码 | 已在 D 的集成树核对：`errors.py` 中两者分别为 `40005`/`40006` |
| 4 | 历史快照 JSON 精度策略 | 按现有 **4/2/6** 落库；C 的结果以 **LONGTEXT 原文**保存（v8），回读不再舍入 | JSON 列会把数值叶子归一化到约 15 位有效数字，实测 `99633.35582084299` → `99633.355820843`，故 v8 改文本列 |
| 5 | C1 白名单最小字段集与预热有效行数 | 单字段 `2 ≤ period ≤ 120`（B 侧 `schemas/backtest.py` 强制）、跨字段 `2 ≤ short < long`（B 侧 `resolve_effective_parameters` 强制）；**预热 = V2 开始日前最后 `long` 条有效日线**（`long=120` 正好 120 条，不是 121） | C1 已定稿；C 复核后 B 已更正文档（原先误写 `max(ma_trend_period, ma_long_period+1)`，那是 V1 legacy 口径） |

> 新增的未决事项不在本节，见 **Issue #15**（运行时 K 线兜底来源）。
